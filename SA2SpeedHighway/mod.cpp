...
static LandTableInfo* CurrentLandInfo = nullptr;
...
static ObjectListHead SpeedHighwayObjListH = { arraylengthandptr(SpeedHighwayObjList) };

static void LoadSHAct(int act)
{
	FreeLandTable(&CurrentLandInfo);
	DeleteSetHandler();

	CurrentAct = act;

	// Delete all set objects here

	switch (act)
	{
	case 0:
		LoadLandTable("resource\\gd_pc\\speed-highway0.sa2lvl", &CurrentLandInfo, &HIGHWAY01_TEXINFO);
		...
		break;
	case 1:
		LoadLandTable("resource\\gd_pc\\speed-highway1.sa2lvl", &CurrentLandInfo, &HIGHWAY02_TEXINFO);
		...
		break;
	case 2:
		LoadLandTable("resource\\gd_pc\\speed-highway2.sa2lvl", &CurrentLandInfo, &HIGHWAY03_TEXINFO);
